...
class PredictionPipeline:
    ...
    def get_model_from_gcloud(self) -> str:
        ...
        Description: This method retrieves best model from the google cloud storage

        Output: best_model_path
        """
        logging.info("Entered the get_model_from_gcloud method of PredictionPipeline")
        try:
            ...
            logging.info("Exited the get_model_from_gcloud method")
    ...
    def predict(self, best_model_path, text):
        """load image returns a CUDA tensor"""
        logging.info("Running the predict function")
        try:
            if os.path.exists('/artifacts/PredictModel/model.h5'):
                print('model already exists') 
                best_model_path:str = 'artifacts/PredictModel/model.h5'
            else:
    ...
    def run_pipeline(self,text):
        logging.info("Entered the run_pipeline method of PredictionPipeline")
        try:
            best_model_path: str = self.get_model_from_gcloud()
            predicted_text = self.predict(best_model_path, text)
            logging.info("Exited the run_pipeline method of PredictionPipeline")
